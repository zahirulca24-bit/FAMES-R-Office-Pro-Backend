...
from app.audit_schemas import (
    ...
    AcceptanceView,
    IndependenceUpsertRequest,
    IndependenceView,
    MaterialityUpsertRequest,
    MaterialityView,
    PlanningReadinessView,
    RiskCreateRequest,
    RiskProcedureLinkRequest,
    RiskView,
)
...
def upsert_acceptance(engagement_id: str, payload: AcceptanceUpsertRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> AcceptanceView:
    ...
    if row is None:
        row = AuditAcceptance(engagement_id=engagement_id, created_by_user_id=user.id)
        db.add(row)
    elif payload.expected_version is None or row.version != payload.expected_version:
        raise ApiError("VERSION_CONFLICT", "Acceptance record has changed; reload before updating", 409, {"current_version": row.version})
    row.continuance = payload.continuance
    row.integrity_assessment = payload.integrity_assessment
    row.competence_resources = payload.competence_resources
    row.preconditions_met = payload.preconditions_met
    row.reason = payload.reason
    row.status = "DRAFT"
    row.version += 1 if row.id else 0
    _event(db, request, user, engagement_id, "AUDIT_ACCEPTANCE_UPDATED", {"status": row.status})
...
def decide_acceptance(engagement_id: str, payload: AcceptanceDecisionRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> AcceptanceView:
    ...
    row = db.scalar(select(AuditAcceptance).where(AuditAcceptance.engagement_id == engagement_id))
    if row is None:
        raise ApiError("AUDIT_ACCEPTANCE_MISSING", "Acceptance assessment must be completed first", 409)
    if row.version != payload.expected_version:
        raise ApiError("VERSION_CONFLICT", "Acceptance record has changed", 409, {"current_version": row.version})
    if payload.decision == "APPROVED" and not row.preconditions_met:
        raise ApiError("AUDIT_PRECONDITIONS_NOT_MET", "Audit preconditions must be met before acceptance approval", 409)
    row.status = payload.decision; row.reason = payload.reason; row.version += 1
    row.approved_by_user_id = user.id if payload.decision == "APPROVED" else None
    row.approved_at = _now() if payload.decision == "APPROVED" else None
    _event(db, request, user, engagement_id, "AUDIT_ACCEPTANCE_DECIDED", {"decision": payload.decision})
    db.commit(); db.refresh(row)
    return AcceptanceView.model_validate(row)
...
def upsert_independence(engagement_id: str, payload: IndependenceUpsertRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> IndependenceView:
    _accessible(db, user, engagement_id, Permission.AUDIT_PLAN)
    member = db.scalar(select(EngagementTeamMember).where(EngagementTeamMember.engagement_id == engagement_id, EngagementTeamMember.staff_id == payload.staff_id, EngagementTeamMember.status == "ACTIVE"))
    if member is None:
        raise ApiError("AUDIT_STAFF_NOT_ON_TEAM", "Independence declaration requires an active engagement team member", 422)
    ...
    if row is None:
        row = AuditIndependenceDeclaration(engagement_id=engagement_id, staff_id=payload.staff_id, declared_by_user_id=user.id); db.add(row)
    ...
    _event(db, request, user, engagement_id, "AUDIT_INDEPENDENCE_DECLARED", {"staff_id": payload.staff_id, "status": payload.status})
    db.commit(); db.refresh(row)
...
def upsert_materiality(engagement_id: str, payload: MaterialityUpsertRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> MaterialityView:
    _accessible(db, user, engagement_id, Permission.AUDIT_PLAN)
    if payload.performance_materiality_minor > payload.overall_materiality_minor:
        raise ApiError("INVALID_MATERIALITY", "Performance materiality cannot exceed overall materiality", 422)
    row = db.scalar(select(AuditMateriality).where(AuditMateriality.engagement_id == engagement_id))
    if row is None:
        row = AuditMateriality(engagement_id=engagement_id, created_by_user_id=user.id, **payload.model_dump(exclude={"expected_version"})); db.add(row)
    else:
        if payload.expected_version is None or row.version != payload.expected_version:
            raise ApiError("VERSION_CONFLICT", "Materiality record has changed", 409, {"current_version": row.version})
        for key, value in payload.model_dump(exclude={"expected_version"}).items(): setattr(row, key, value)
        row.version += 1
    _event(db, request, user, engagement_id, "AUDIT_MATERIALITY_UPDATED", {"overall_materiality_minor": payload.overall_materiality_minor})
    db.commit(); db.refresh(row)
...
def create_risk(engagement_id: str, payload: RiskCreateRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> RiskView:
    _accessible(db, user, engagement_id, Permission.AUDIT_PLAN)
    duplicate = db.scalar(select(AuditRisk.id).where(AuditRisk.engagement_id == engagement_id, AuditRisk.risk_code == payload.risk_code))
    if duplicate:
        raise ApiError("AUDIT_RISK_DUPLICATE", "Risk code already exists for this engagement", 409)
    row = AuditRisk(engagement_id=engagement_id, created_by_user_id=user.id, **payload.model_dump()); db.add(row)
    _event(db, request, user, engagement_id, "AUDIT_RISK_CREATED", {"risk_code": payload.risk_code})
    db.commit(); db.refresh(row)
...
def link_risk_procedure(risk_id: str, payload: RiskProcedureLinkRequest, request: Request, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> dict[str, str]:
    ...
    _accessible(db, user, risk.engagement_id, Permission.AUDIT_PLAN)
    paper = db.get(WorkingPaper, payload.working_paper_id)
    if paper is None or paper.engagement_id != risk.engagement_id:
        raise ApiError("AUDIT_PROCEDURE_ENGAGEMENT_MISMATCH", "Working paper must belong to the same engagement", 422)
    exists = db.scalar(select(AuditRiskProcedure.id).where(AuditRiskProcedure.risk_id == risk.id, AuditRiskProcedure.working_paper_id == paper.id))
    if exists: raise ApiError("AUDIT_PROCEDURE_DUPLICATE", "Working paper already linked to this risk", 409)
    row = AuditRiskProcedure(risk_id=risk.id, working_paper_id=paper.id, procedure_note=payload.procedure_note); db.add(row)
    _event(db, request, user, risk.engagement_id, "AUDIT_RISK_PROCEDURE_LINKED", {"risk_id": risk.id, "working_paper_id": paper.id})
    db.commit(); db.refresh(row)
...
def planning_readiness(engagement_id: str, db: Session = Depends(get_db), user: AuthUser = Depends(require_password_changed)) -> PlanningReadinessView:
    ...
    threats = sum(1 for row in declarations if row.status != "CLEAR")
    risks = list(db.scalars(select(AuditRisk).where(AuditRisk.engagement_id == engagement_id)))
    without_procedure = 0
    for risk in risks:
        if db.scalar(select(AuditRiskProcedure.id).where(AuditRiskProcedure.risk_id == risk.id).limit(1)) is None: without_procedure += 1
    independence_complete = active_team > 0 and len(declarations) >= active_team and threats == 0
    acceptance_approved = acceptance is not None and acceptance.status == "APPROVED"
    materiality_set = materiality is not None and materiality.overall_materiality_minor > 0
